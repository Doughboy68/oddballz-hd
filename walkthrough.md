...
Enhanced `d:/antigravity/oddballz-hd` with real-time 3D hexagonal gravity drop path animations, zip-speed fall physics for unsupported balls breaking off from locked shapes, smooth piece lock-in, full horizontal transformation precision, ergonomic mobile UI button height scaling, and mathematical 3D playfield centering.

## Recent Enhancements

### 1. 3D Playfield Horizontal Centering
- **Mathematical Center-of-Mass Alignment**: Corrected the `-0.75` horizontal offset in `gridToWorld()` ([hexMath.js](file:///d:/antigravity/oddballz-hd/src/engine/hexMath.js#L29-L41) and [oddballz-app.js](file:///d:/antigravity/oddballz-hd/oddballz-app.js#L19-L26)). Adding `+0.75` to `worldX` aligns the board's average center of mass exactly to $X = 0.0$, centering the entire 3D playfield symmetrically between left and right screen margins on mobile and desktop viewports.

### 2. Mobile UI Vertical Button Scaling
...
### 3. Shape Break-Apart Zip Drop Animation
...
### 4. Clean Lock-In State Transition
...
### 5. Horizontal Transformation Precision Fix
